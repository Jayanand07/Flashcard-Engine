    ...
    <div className="mx-auto w-full cursor-pointer select-none" style={{ perspective: "1200px", maxWidth: "680px" }} onClick={onFlip}>
      <div className="relative w-full" style={{
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center overflow-hidden rounded-2xl px-10 py-12 sm:px-14 transition-shadow duration-500"
          ...
          <span className="absolute left-6 top-6 text-[10px] font-semibold uppercase tracking-[0.2em]" style={{ color: "var(--accent)" }}>Question</span>
          <p className="max-h-[340px] overflow-y-auto text-center text-xl font-semibold leading-relaxed sm:text-2xl" style={{ color: "var(--text-primary)" }}>{question}</p>
          <span className="absolute bottom-6 text-xs animate-subtle-pulse" style={{ color: "var(--text-secondary)" }}>tap anywhere to flip</span>
        </div>
        {/* Back */}
        <div className="absolute inset-0 flex flex-col items-center justify-center overflow-hidden rounded-2xl px-10 py-12 sm:px-14 transition-shadow duration-500"
          ...
          <span className="absolute left-6 top-6 text-[10px] font-semibold uppercase tracking-[0.2em]" style={{ color: "var(--success)" }}>Answer</span>
          <p className="max-h-[360px] overflow-y-auto text-center text-lg leading-relaxed" style={{ color: "var(--text-primary)" }}>{answer}</p>
        </div>
      </div>
    </div>
  );
